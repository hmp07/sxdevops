...
class GrafanaSetting(models.Model):
    ...
    api_token = models.CharField('API Token', max_length=256, blank=True, default='', help_text='Grafana Service Account Token（glsa_...），服务端调用 Grafana API 使用')
    org_id = models.PositiveIntegerField('Org ID', default=1, help_text='嵌入 URL 的 orgId 参数')
    tls_verify = models.BooleanField('TLS 验证', default=True)
    timeout = models.PositiveIntegerField('超时(秒)', default=10)
    jwt_secret = models.CharField('JWT Secret', max_length=256, blank=True, default='', help_text='与 Grafana [auth.jwt] 共享的 HS256 secret，用于签发 iframe 嵌入的 auth_token')
    updated_by = models.CharField('更新人', max_length=64, blank=True, default='')
    created_at = models.DateTimeField('创建时间', auto_now_add=True)
    updated_at = models.DateTimeField('更新时间', auto_now=True)
